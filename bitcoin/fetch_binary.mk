...
BITCOIN_DEPS=$(BITCOIN_STRATEGY_DEPS) $(BITCOIN_FETCH_FILES)
BITCOIN_FETCH_FILES=$(BITCOIN_BIN_ARCHIVE) $(BUILD_DIR)/bitcoin-SHA256SUMS.asc $(BUILD_DIR)/laanwj.gpg

$(BUILD_DIR)/verify-bitcoin.stamp: $(BITCOIN_BIN_ARCHIVE) $(BUILD_DIR)/bitcoin-SHA256SUMS
	cd $(BUILD_DIR) && sha256sum --check bitcoin-SHA256SUMS
	tar -C $(BUILD_DIR) -xzmf $<
	touch $@

$(BUILD_DIR)/bitcoin-$(BITCOIN_VERSION)/Makefile: $(BITCOIN_SOURCE_DIR)assets/bin-makefile $(BUILD_DIR)/verify-bitcoin.stamp
	cp $< $@
